#!/usr/bin/env bash
# SMART_AO V7.1 - check_go_nogo_fleet.sh
# Gate bloquant : 46 critères Fleet Multi-VPS avant premier client payant
set -euo pipefail
...
echo "============================================================"
echo "SMART_AO V7.1 - GO/NO-GO VALIDATION - 46 Fleet Multi-VPS"
echo "============================================================"
...
check_file "app/engines/fleet_engine/__init__.py" "app/engines/fleet_engine/__init__.py"
...
# === Fleet Multi-VPS spécifique (6) ===
check_file "Fleet Engine updater.py" "app/engines/fleet_engine/updater.py"
check_file "Fleet Engine license_checker.py" "app/engines/fleet_engine/license_checker.py"
check_file "Fleet Engine cosign_verifier.py" "app/engines/fleet_engine/cosign_verifier.py"
check "Fleet licence liée au VPS" python3 -c "
import os
os.environ['COSIGN_MOCK'] = '1'
from datetime import datetime, timezone, timedelta
from app.engines.fleet_engine import LicenseChecker, FleetUpdater, CosignVerifier
checker = LicenseChecker(secret='FLEET_TEST')
exp = datetime.now(timezone.utc) + timedelta(days=30)
lic_vps_a = checker.issue_license('vps-a', '123', exp)
assert checker.validate(lic_vps_a, expected_vps_id='vps-a')['valid'] is True
assert checker.validate(lic_vps_a, expected_vps_id='vps-b')['valid'] is False
"
check "Fleet cosign verify image" python3 -c "
import os
os.environ['COSIGN_MOCK'] = '1'
from app.engines.fleet_engine import CosignVerifier
v = CosignVerifier()
assert v.verify('smart-ao/engine:v7.1.1').verified is True
assert v.verify('untrusted/image:latest').verified is False
"
check "Fleet pull-based update applicable" python3 -c "
import os
os.environ['COSIGN_MOCK'] = '1'
from datetime import datetime, timezone, timedelta
from app.engines.fleet_engine import LicenseChecker, FleetUpdater, CosignVerifier
checker = LicenseChecker(secret='FLEET_TEST')
exp = datetime.now(timezone.utc) + timedelta(days=30)
lic = checker.issue_license('vps-fleet-01', '12345678900012', exp)
...
manifest = {'version': 'v7.1.1', 'image': 'smart-ao/engine:v7.1.1'}
status = updater.check_for_update(current_version='v7.1.0', manifest=manifest)
assert status.applicable is True
assert updater.apply_update(status)['success'] is True
"
...
echo "============================================================"
if [ "$FAILED" -eq 0 ]; then
    echo "GO: Structure et tests Fleet V7.1 valides ($PASSED/$TOTAL)"
    exit 0
else
